...
<ul class="navbar-nav mr-auto">
  ...
  <li class="nav-item dropdown active">
      ...
      <div class="dropdown-menu" aria-labelledby="navbarDropdown">
        ...
        <a class="dropdown-item" href="#">รายงานจำนวนการใช้งาน (จำแนกตามภาควิชา)</a>
...
<body class="bg">
         ...
         <!-- jQuery first, then Popper.js, then Bootstrap JS -->
          ...
          <h3 align='center'style="margin-top:30px;">รายงานจำนวนการใช้งาน (จำแนกตามภาควิชา)</h3>
          <div id ="body" class="body">
            <form method="post" name="selected" action="ad_report2.php">
          <div class="row" style="margin:12px;">
            ...
            <div class="col-2">
              <input type="text" id="myInput" oninput="myFunction()" class="form-control" placeholder="กรอกชื่อภาควิชา" autocomplete="off">
            </div>
            <div class="col"></div>
          </div>
        </div>
          <center><div class="container" id="my" style="overflow-y:scroll; height: 350px; margin-top:30px;">
      ...
      foreach ($dep as $rw) {
        ...
        for ($n=0; $n < count($rooms); $n++) {
          if (isset($_POST['submit'])&&$_POST['date']!='') {
            $sql = "SELECT COUNT(user.dep_id) c FROM res INNER JOIN user ON res.user_id = user.user_id
            WHERE  res.room_id = '".$rooms[$n]."' AND user.dep_id ='".$rw['id']."'AND res.r_date BETWEEN '".$_REQUEST['date']."'AND '".$_REQUEST['date2']."' ";
          }
          else {
            $sql = "SELECT COUNT(user.dep_id) c FROM res INNER JOIN user ON res.user_id = user.user_id
            WHERE  res.room_id = '".$rooms[$n]."' AND user.dep_id ='".$rw['id']."'";
          }
          $res = $con->query($sql);
          $row = $res->fetch_assoc();
          echo '<td>'.$row['c'].'</td>';
        ...
        if (isset($_POST['submit'])&&$_POST['date']!='') {
          $sql = "SELECT COUNT(user.dep_id) c FROM res INNER JOIN user ON res.user_id = user.user_id
          WHERE  user.dep_id = '".$rw['id']."' AND res.r_date BETWEEN '".$_REQUEST['date']."'AND '".$_REQUEST['date2']."'";
        }
        else {
          $sql = "SELECT COUNT(user.dep_id) c FROM res INNER JOIN user ON res.user_id = user.user_id
          WHERE  user.dep_id = '".$rw['id']."'";
        }
        $res = $con->query($sql);
        $row = $res->fetch_assoc();
        echo '<td>'.$row['c'].'</td>';
      ...
      <tr>
        ...
        if (isset($_POST['submit'])&&$_POST['date']!='') {
          $sql = "SELECT COUNT(user.dep_id) c FROM res INNER JOIN user ON res.user_id = user.user_id
          WHERE  res.room_id ='".$rooms[$n]."' AND res.r_date BETWEEN '".$_REQUEST['date']."'AND '".$_REQUEST['date2']."'";
        }
        else {
          $sql = "SELECT COUNT(user.dep_id) c FROM res INNER JOIN user ON res.user_id = user.user_id
          WHERE  res.room_id ='".$rooms[$n]."'";
        }
        $res = $con->query($sql);
        $row = $res->fetch_assoc();
        echo '<td>'.$row['c'].'</td>';
        ...
        if (isset($_POST['submit'])&&$_POST['date']!='') {
            $sql = "SELECT COUNT(user.status) c FROM res INNER JOIN user ON res.user_id = user.user_id WHERE res.r_date BETWEEN '".$_REQUEST['date']."'AND '".$_REQUEST['date2']."'";
        }
        else {
          $sql = "SELECT COUNT(user.status) c FROM res INNER JOIN user ON res.user_id = user.user_id";
        }
        $res = $con->query($sql);
        $row = $res->fetch_assoc();
        echo '<td>'.$row['c'].'</td>';
