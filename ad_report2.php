...
<ul class="navbar-nav mr-auto">
  ...
  <li class="nav-item dropdown active">
      ...
      <div class="dropdown-menu" aria-labelledby="navbarDropdown">
        ...
        <a class="dropdown-item" href="#">รายงานจำนวนการใช้งาน (จำแนกตามกลุ่มผู้ใช้)</a>
...
<body class="bg">
         ...
         <!-- jQuery first, then Popper.js, then Bootstrap JS -->
          ...
          <h3 align='center'style="margin-top:30px;">รายงานจำนวนการใช้งาน (จำแนกตามกลุ่มผู้ใช้)</h3>
      ...
      <table class="table  table-bordered " id="report" style="margin-bottom: 30px;">
        <tr class="text-white bg-success">
          ...
          <th> อาจารย์ </th>
          <th>เจ้าหน้าที่</th>
          <th>รวมจำนวนการใช้งาน</th>
        </tr>
      </div></center>
      <?php
      $i = 1;
      $status = array('นักศึกษา','อาจารย์','เจ้าหน้าที่');
      foreach ($room as $rw) {
        echo "<tr>";
        echo '<td>'.$i.'</td>';
        echo '<td>'.$rw['name'].'</td>';
        for ($n=0; $n < count($status); $n++) {
          if (isset($_POST['submit'])&&$_POST['date']!='') {
            ...
            WHERE  res.room_id = '".$rw['id']."' AND user.status ='".$status[$n]."'AND res.r_date BETWEEN '".$_REQUEST['date']."'AND '".$_REQUEST['date2']."' ";
          }
          else {
            $sql = "SELECT COUNT(user.status) c FROM res INNER JOIN user ON res.user_id = user.user_id
            WHERE  res.room_id = '".$rw['id']."' AND user.status ='".$status[$n]."'";
          }
          $res = $con->query($sql);
          $row = $res->fetch_assoc();
          echo '<td>'.$row['c'].'</td>';
        ...
        if (isset($_POST['submit'])&&$_POST['date']!='') {
          $sql = "SELECT COUNT(user.status) c FROM res INNER JOIN user ON res.user_id = user.user_id
          WHERE  res.room_id = '".$rw['id']."' AND res.r_date BETWEEN '".$_REQUEST['date']."'AND '".$_REQUEST['date2']."'";
        }
        else {
          $sql = "SELECT COUNT(user.status) c FROM res INNER JOIN user ON res.user_id = user.user_id
          WHERE  res.room_id = '".$rw['id']."'";
        }
        $res = $con->query($sql);
        $row = $res->fetch_assoc();
        echo '<td>'.$row['c'].'</td>';
      ...
      <tr>
        ...
        if (isset($_POST['submit'])&&$_POST['date']!='') {
          $sql = "SELECT COUNT(user.status) c FROM res INNER JOIN user ON res.user_id = user.user_id
          WHERE  user.status ='".$status[$n]."' AND res.r_date BETWEEN '".$_REQUEST['date']."'AND '".$_REQUEST['date2']."'";
        }
        else {
          $sql = "SELECT COUNT(user.status) c FROM res INNER JOIN user ON res.user_id = user.user_id
          WHERE  user.status ='".$status[$n]."'";
        }
        $res = $con->query($sql);
        $row = $res->fetch_assoc();
        echo '<td>'.$row['c'].'</td>';
