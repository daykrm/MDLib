...
$time = array('08.30-12.00','13.00-16.30');
?>
<html>
<head>
...
<ul class="navbar-nav mr-auto">
  ...
  <li class="nav-item dropdown active">
      ...
      <div class="dropdown-menu" aria-labelledby="navbarDropdown">
        ...
        <a class="dropdown-item" href="#">รายงานจำนวนการใช้งาน (จำแนกตามช่วงเวลา)</a>
...
<body class="bg">
         ...
         <!-- jQuery first, then Popper.js, then Bootstrap JS -->
          ...
          <h3 align='center'style="margin-top:30px;">รายงานจำนวนการใช้งาน (จำแนกตามช่วงเวลา)</h3>
          <div id ="body" class="body">
            <form method="post" name="selected" action="ad_report4.php">
      ...
      foreach ($room as $rw) {
        ...
        foreach ($time as $key) {
          if (isset($_POST['date'])&&$_POST['date']!='') {
            $sql = "SELECT COUNT(r_time) c FROM res WHERE r_time = '".$key."' AND room_id ='".$rw['id']."' AND r_date BETWEEN '".$_REQUEST['date']."'AND '".$_REQUEST['date2']."'";
          }
          else {
            $sql = "SELECT COUNT(r_time) c FROM res WHERE r_time = '".$key."' AND room_id ='".$rw['id']."'";
          }
          $res = $con->query($sql);
          $row = $res->fetch_assoc();
          echo '<td>'.$row['c'].'</td>';
      ...
      <tr>
        ...
        foreach ($time as $key) {
          if (isset($_POST['date'])&&$_POST['date']!='') {
            $sql = "SELECT COUNT(r_time) c FROM res WHERE r_time = '".$key."' AND r_date BETWEEN '".$_REQUEST['date']."'AND '".$_REQUEST['date2']."'";
          }
          else {
            $sql = "SELECT COUNT(r_time) c FROM res WHERE r_time = '".$key."'";
          }
        $res = $con->query($sql);
        $row = $res->fetch_assoc();
        echo '<td>'.$row['c'].'</td>';
